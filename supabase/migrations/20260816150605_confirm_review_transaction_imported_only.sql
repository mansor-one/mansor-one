-- Keep Review Queue promotion compatible with the authenticated column-level
-- privilege contract. The normal promotion path only needs to mark the source
-- import as imported. The ignore branch is intentionally unchanged and must be
-- audited separately because it also changes transaction_status.
