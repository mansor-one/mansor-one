-- Promote one legacy scheduled-payment occurrence into the canonical obligation
-- lifecycle without deleting either the schedule or its financial evidence.

create unique index if not exists obligation_payment_links_one_reconciled_quick_entry
  on public.obligation_payment_links(quick_entry_id)
  where quick_entry_id is not null and reconciliation_status = 'reconciled';
...
begin
  ...
  where id = p_quick_entry_id
    and household_id = v_household_id
    and lower(coalesce(entry_type, '')) not in ('income', 'transfer')
  for update;
...
-- Rollback: revoke/drop configure_legacy_paid_obligation and drop the partial
-- quick-entry index only after confirming no reconciled links rely on it.
