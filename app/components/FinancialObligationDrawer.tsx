    ...
    <aside className="mobile-safe-drawer h-full w-full max-w-2xl overflow-y-auto border-l border-white/10 bg-[#08101f] p-5 shadow-2xl outline-none sm:p-7" ref={drawerRef} tabIndex={-1}>
      ...
      {details && <div className="mt-6 space-y-6">
        ...
        {detectedLinks.length > 0 && <section><h3 className="mb-3 font-bold">Evidencia candidata</h3><div className="space-y-3">{detectedLinks.map((candidate) => {
          ...
          const debtReductionCredit = Array.isArray(candidate.score_factors) && candidate.score_factors.some((factor: Record<string, unknown>) => factor.code === 'debt_reduction_credit' && factor.passed === true)
          const partialCredit = debtReductionCredit && !exactAmount && candidateAmount < expectedAmount
          const fullCredit = debtReductionCredit && candidateAmount >= expectedAmount
