...
export default function MobileNavigation() {
  ...
  return <>
    <div className="mobile-bottom-nav lg:hidden" aria-label="Navegación principal móvil">
      {tabs.map((item) => {
        const selected = active(pathname, item.href)
        return <Link aria-current={selected ? 'page' : undefined} className={selected ? 'mobile-nav-item mobile-nav-item-active' : 'mobile-nav-item'} href={item.href} key={item.href}><span aria-hidden="true" className="text-lg leading-none">{item.icon}</span><span>{item.label}</span></Link>
      })}
      <button aria-expanded={open} aria-haspopup="dialog" className={moreActive || open ? 'mobile-nav-item mobile-nav-item-active' : 'mobile-nav-item'} onClick={() => setOpen(true)} ref={triggerRef} type="button"><span aria-hidden="true" className="text-lg leading-none">•••</span><span>Más</span></button>
    </div>
    {open && <div aria-labelledby="mobile-more-title" aria-modal="true" className="fixed inset-0 z-[70] lg:hidden" role="dialog">
      <button aria-label="Cerrar menú" className="absolute inset-0 bg-black/70" onClick={() => setOpen(false)} type="button" />
