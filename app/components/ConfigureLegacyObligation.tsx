...
export default function ConfigureLegacyObligation({ payment }: { payment: PaymentInstance }) {
  ...
  async function submit(formData: FormData) {
    ...
    const response = await fetch('/api/obligations/configure-legacy', {
      ...
      body: JSON.stringify({
        ...
        quickEntryId: formData.get('quickEntryId'),
      }),
    })
    const payload = await response.json()
    setSaving(false)
  ...
  return <form action={submit} className="space-y-4 rounded-xl border border-amber-700/60 bg-amber-950/15 p-4">
    ...
    <div className="grid gap-3 sm:grid-cols-2">
      ...
      <label className="text-sm">Importe esperado<input className="mt-1 w-full rounded border border-white/15 bg-[#07101f] p-2" name="amount" type="number" min="0.01" step="0.01" onChange={(event) => setConfiguredAmount(Number(event.target.value))} value={configuredAmount} required /></label>
    ...
    <div className="space-y-2"><label className="block text-sm">Buscar transacción confirmada<div className="mt-1 flex gap-2"><input autoComplete="off" className="min-w-0 flex-1 rounded border border-white/15 bg-[#07101f] p-2" name="legacyTransactionSearch" onChange={(event) => setCandidateSearch(event.target.value)} placeholder="Fecha, descripción, cuenta o importe" type="search" value={candidateSearch}/>{candidateSearch && <button className="rounded border border-white/15 px-3 py-2 text-sm" onClick={() => setCandidateSearch('')} type="button">Limpiar</button>}</div></label><p className="text-xs text-slate-400">Buscamos en el historial confirmado hasta {setup.searchWindowDays} días antes y después del vencimiento. {setup.candidates.length} transacciones están disponibles.</p></div>
    ...
    <label className="block text-sm">Transacción real pagada<select className="mt-1 w-full rounded border border-white/15 bg-[#07101f] p-2" name="quickEntryId" onChange={(event) => setSelectedCandidateId(event.target.value)} value={selectedCandidateId} required><option value="" disabled>Selecciona una transacción confirmada</option>{visibleCandidates.candidates.map((candidate) => <option key={candidate.id} value={candidate.id}>{candidate.entry_date} · {money(candidate.amount)} · {candidate.description}{candidate.financialImpact === 'statement_credit' ? ' · crédito al estado de cuenta' : candidate.exactAmount ? ' · importe exacto' : ' · importe diferente'}</option>)}</select></label>
    {selectedCandidate && Math.abs(Math.abs(selectedCandidate.amount) - configuredAmount) >= 0.01 && <p className="rounded border border-amber-600 bg-amber-950/30 p-3 text-sm text-amber-100"><strong>Advertencia:</strong> la transacción seleccionada es por {money(selectedCandidate.amount)}, pero la obligación está configurada por {money(configuredAmount)}. Puedes continuar porque la selección es manual; verifica que realmente corresponda a este pago.</p>}
    ...
    <p className="text-xs text-slate-400">No se creará ninguna transacción. La opción seleccionada quedará enlazada como evidencia permanente.</p>
    {error && <p className="rounded border border-red-800 p-3 text-red-100">{error}</p>}
    <button className="rounded border border-emerald-600 px-3 py-2 font-semibold text-emerald-100 disabled:opacity-50" disabled={saving || setup.candidates.length === 0} type="submit">{saving ? 'Guardando…' : 'Migrar y conciliar pago'}</button>
  </form>
}
