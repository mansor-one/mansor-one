...
function PaymentCard({ payment }: { payment: TrustedPayment }) {
  const lifecycleLabel = ({
    reconciled: 'Conciliado', pending_settlement: 'Pagado, esperando confirmación',
    payment_detected: 'Pago detectado', cancelled: 'Cancelado',
  } as Record<string, string>)[payment.lifecycleState || ''] || ({
    due_soon: 'Próximo a vencer', overdue: 'Vencido', in_transit: 'Pagado, esperando confirmación',
    possible_match: 'Pago detectado', matched: 'Conciliado', paid: 'Conciliado',
    future: 'Programado', unpaid: 'Programado', due_today: 'Próximo a vencer', grace_period: 'En período de gracia',
  } as Record<string, string>)[payment.truthStatus] || payment.lifecycleLabel || payment.truthStatus.replaceAll('_', ' ')
  return <div className="rounded border p-4 text-sm">
    ...
    <p><span className="inline-flex rounded-full border px-2 py-1 font-semibold">{lifecycleLabel}</span></p>
...
export default async function TimelinePage({ searchParams }: { searchParams: Promise<{ horizon?: string; view?: string; month?: string }> }) {
  ...
  return <AppShell header={{ eyebrow: 'Calendario de efectivo', title: 'Pagos', subtitle: `Obligaciones desde hoy hasta ${formatDate(projection.horizonEnd)}. Las coincidencias posibles requieren confirmación.` }}>
    <nav className="flex flex-wrap gap-2" aria-label="Planning horizon">
      {[30, 45, 90, 365].map((days) => <Link className={`rounded border px-3 py-2 text-sm ${days === horizonDays ? 'font-bold' : ''}`} href={`/timeline?horizon=${days}${query.view ? `&view=${query.view}` : ''}${query.month ? `&month=${query.month}` : ''}#dashboard-calculation`} key={days}>{days === 365 ? 'Full year' : `${days} days`}</Link>)}
    </nav>
    ...
    {(isActionableDrilldown || isOpenDrilldown || isAdjustedRiskDrilldown || isIncomeDrilldown) && (
      <section id="dashboard-calculation" className="space-y-3 rounded border border-blue-500 p-4">
        ...
        <div className="grid gap-3 lg:grid-cols-2">
          {!isIncomeDrilldown
            ? drilldownPayments.map((payment) => <PaymentCard payment={payment} key={payment.id} />)
            : incomeEvents.map((event) => <EventCard event={event} key={`${event.type}:${event.id}`} />)}
        </div>
      </section>
    )}
    ...
    <EventSection title="Requieren atención" events={sections.needsAttention} empty="No hay pagos vencidos, para hoy o dentro de gracia en este período." />
    <EventSection title="Próximos" events={sections.upcoming} empty="No hay pagos o ingresos próximos listos para proyectar." />
    <EventSection title="Coincidencias posibles" events={sections.possibleMatches} empty="No hay coincidencias inciertas que requieran confirmación." />

    <section id="in-transit" className="space-y-3"><h2 className="text-xl font-bold">Pagos esperando confirmación</h2>{sections.inTransit.length ? <div className="grid gap-3 lg:grid-cols-2">{sections.inTransit.map((payment) => <PaymentCard payment={payment} key={payment.id} />)}</div> : <p className="text-sm opacity-70">No hay pagos recientes esperando confirmación bancaria.</p>}</section>

    <section id="payments" className="space-y-3"><h2 className="text-xl font-bold">Pagados o conciliados</h2>{sections.paidOrReconciled.length ? <div className="grid gap-3 lg:grid-cols-2">{sections.paidOrReconciled.map((payment) => <PaymentCard payment={payment} key={payment.id} />)}</div> : <p className="text-sm opacity-70">No hay pagos conciliados en este período.</p>}</section>
    <section className="space-y-3"><h2 className="text-xl font-bold">Más adelante</h2>{sections.later.length ? <div className="grid gap-3 lg:grid-cols-2">{sections.later.map((payment) => <PaymentCard payment={payment} key={payment.id} />)}</div> : <p className="text-sm opacity-70">No hay pagos fuera de este horizonte.</p>}</section>
